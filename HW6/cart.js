// Пытался сам накреативить, но получалась бяка, неправильно использовал EventListener, и отрисовку, так что тут просто Ваш код, так как лучше решения не нашёл, но его проштудировал и понял.
...
function drawGoods() {
    goods.forEach(function (good, i) {
        ...
        const html = `<div class="good"><h5>${good.name}</h5><p>${good.price}</p>${imagesHtml}<button data-id="${i}">+</button></div>`;
